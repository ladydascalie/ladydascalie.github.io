...
function initSkillBars() {
        ...
        function animateSkillBars() {
                skillBars.forEach((bar, index) => {
                        const progress =
                                bar.getAttribute("data-progress") || "50";
...
function initGuestbook() {
        ...
        if (guestForm) {
                guestForm.addEventListener("submit", (e) => {
                        ...
                        const email = guestForm.querySelector(
                                'input[type="email"]',
                        ).value;
                        const message =
                                guestForm.querySelector("textarea").value;
...
function initRainbowEffects() {
        const rainbowElements = document.querySelectorAll(
                ".rainbow-link, .title",
        );
...
function initSoundEffects() {
        ...
        const buttons = document.querySelectorAll(
                ".contact-btn, .submit-btn, .webring-btn",
        );
...
function showNotification(message, type = "info", duration = 4000) {
        ...
        background: ${
                type === "error"
                        ? "linear-gradient(45deg, #ff0040, #ff6b00)"
                        : type === "success"
